        ...
        for(WeaponMount mount : mounts){
            ...
            if(weapon.rotate && (mount.rotate || mount.shoot) && can){
                ...
                if(weapon.rotateLimit > 0){
                    mount.targetRotation = Mathf.clamp(Angles.angle(axisX, axisY, mount.aimX, mount.aimY) - this.rotation, weaponRotation - weapon.rotateLimit, weaponRotation + weapon.rotateLimit);
                }
                else{
