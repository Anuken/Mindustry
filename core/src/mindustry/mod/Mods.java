        ...
        for(Fi file : sprites){
            ...
            tasks.add(async.submit(() -> {
                try{
                    ...
                    return () -> {
                        String name = (prefix ? mod.name + "-" : "") + file.nameWithoutExtension();
                        packer.add(getPage(file), name, new PixmapRegion(pix));
                        if(textureScale != 1.0f){ Drawf.textureResize.put(name, textureScale); }
                        pix.dispose();
                    };
                }catch(Exception e){
