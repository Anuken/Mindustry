...
public interface Autotiler{
    ...
    default int[] buildBlending(Tile tile, int rotation, BuildPlan[] directional, boolean world){
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(blends(tile, rotation, directional, i, world) && (tile != null && tile.getNearbyBuilding(realDir) != null && !tile.getNearbyBuilding(realDir).block.squareSprite)){
                blendresult[4] |= (1 << i);
            }
        }
    ...
    default boolean blends(Tile tile, int rotation, int direction){
        Building other = tile.getNearbyBuilding(Mathf.mod(rotation - direction, 4));
