...
public class ItemSelection{
    ...
    public static <T extends UnlockableContent> void buildTable(Table table, Array<T> items, Prov<T> holder, Cons<T> consumer){
        ...
        for(T item : items){
            if(!showAll && !data.isUnlocked(item) && world.isZone()) continue;
    ...
    public static <T extends UnlockableContent> void buildTable(Table table, Array<T> items, Prov<T> holder, Cons<T> consumer, boolean _showAll){
        showAll = _showAll;
        buildTable(table, items, holder, consumer);
        showAll = false;
    }
}
