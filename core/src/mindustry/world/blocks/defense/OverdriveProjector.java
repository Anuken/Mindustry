...
public class OverdriveProjector extends Block{
    ...
    public class OverdriveBuild extends Building implements Ranged{
        ...
        public void draw(){
            ...
            for(int i = 0; i < 4; i++){
                Lines.lineAngleCenter(x + Geometry.d8edge(i).x * size * tilesize / 2f * (1f - f), y + Geometry.d8edge(i).y * size * tilesize / 2f * (1f - f), 135f + 90f * i, hype * (1f - Math.abs(f - 0.5f)) * 2f);
                if(f < 0.5f) Lines.lineAngleCenter(x + Geometry.d4(i).x * size * tilesize / 2f, y + Geometry.d4(i).y * size * tilesize / 2f, 90f + 90f * i, (0.5f - f) * 2f * size * tilesize);
            }
