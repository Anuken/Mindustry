...
public class Projector extends Block{
    ...
    public Projector(String name){
        ...
        hasItems = true;
        emitLight = true;
        if(overdrive) canOverdrive = false;
        lightRadius = 50f;
    }
    ...
    public void setBars(){
        super.setBars();
        if(overdrive) bars.add("boost", (ProjectorBuild entity) -> new Bar(() -> Core.bundle.format("bar.boost", Math.max((int)(entity.realBoost() * 100 - 100), 0)), () -> Pal.accent, () -> entity.realBoost() / (hasBoost ? speedBoost + speedBoostPhase : speedBoost)));
    }

    public class ProjectorBuild extends Building implements Ranged{
        float heat;
        ...
        public void updateTile(){
            ...
            if(charge >= reload){
                ...
                if(overdrive) indexer.eachBlock(this, realRange, other -> true, other -> other.applyBoost(realBoost(), reload + 1f));
            }
        }
